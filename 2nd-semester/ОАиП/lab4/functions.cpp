...
void get_string(char* str, int max)												//функция ввода строки
{
	if (str)
	{
		rewind(stdin);															//очистка буфера
		fgets(str, max, stdin);
		*(str + get_length(str) - 1) = '\0';									//размещение нуль-символа в конце строки
	}
}
...
void input_array(cube* array, int n)											//функция заполнения полей массива кубов
{
	...
	for (i = 0; i < n; i++)
	{
		...
		get_string((array + i)->colour, 15);									//ввод цвета i-ого куба
		get_string((array + i)->material, 15);									//ввод материала i-ого куба
	}
}
...
void txt_output(cube* cubes, int k, int mode)									//функция записи таблицы в текстовый файл
{
	FILE* f;

	int i = 0;

	if (mode)																	//выбор: запись или дозапись
	{
		fopen_s(&f, "text.txt", "a");											//открытие файла для дозаписи
		if (!f) return;
	}
	else
	{
		fopen_s(&f, "text.txt", "w");											//открытие файла для записи
		if (!f) return;
		fputs("Длина ребра: | Цвет:      | Материал:\n\n", f);					//шапка таблицы 
	}

	for (i = 0; i < k; i++)
	{
		fprintf(f, "%-3d см       | %-10s | %-10s\n", (cubes + i)->length, (cubes + i)->colour, (cubes + i)->material);	//запись очередной структуры
	}

	fclose(f);																	//закрытие файла
}

cube* bin_input(int *k, char* fname)											//функция чтения массива из бинарного файла
{
	FILE* f;
	
	cube* cubes;																//массив структур

	fopen_s(&f, fname, "rb");													//открытие файла для чтения
	if (!f) return 0;

	fseek(f, 0, SEEK_END);														//установление указателя позиции в конец файла					
	*k = (int)(ftell(f)) / sizeof(cube);										//нахождение кол-ва записанных структур
	cubes = allocate_memory(*k);												//выделение памяти для массива структур

	fseek(f, 0, SEEK_SET);														//установление указателя позиции в начало файла
	fread(cubes, sizeof(cube), *k, f);											//чтение k структур

	fclose(f);																	//закрытие файла
	return cubes;
}

void bin_output(cube* cubes, int k, char* fname, int mode)						//функция дозаписи массива в бинарный файл
{
	FILE* f;

	if (mode)																	//выбор: запись или дозапись
		fopen_s(&f, "binary.bin", "ab");										//открытие файла для дозаписи
	else
		fopen_s(&f, "binary.bin", "wb");										//открытие файла для записи
	if (!f) return;

	fwrite(cubes, sizeof(cube), k, f);											//запись k структур из массива структур
	fclose(f);																	//закрытие файла
}
	...
	for (i = 0; i < k; i++)
	{
		for (j = 0; j < n; j++)
			if (compare((cubes + i)->colour, *(colours + j)))
			{
				...
				*(*(array + j) + 1) += (int)pow((cubes + i)->length, 3);		//суммирование объёма очередного куба с общим объёмом
			}
	}
