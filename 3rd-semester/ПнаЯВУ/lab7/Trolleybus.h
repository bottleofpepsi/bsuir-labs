...
public:
	...
	bool operator== (const Trolleybus&);							//прототип оператора равно
	bool operator>(const Trolleybus&);								//прототип оператора больше
	bool operator<(const Trolleybus&);								//прототип оператора меньше

	friend istream& operator>>(istream&, Trolleybus&);				//прототип оператора ввода
	friend ostream& operator<<(ostream&, Trolleybus&);				//прототип оператора вывода
	friend ifstream& operator>>(ifstream&, Trolleybus&);			//прототип оператора чтения из файла
	friend ofstream& operator<<(ofstream&, Trolleybus&);			//прототип оператора записи в файл
	friend fstream& operator>>(fstream&, Trolleybus&);				//прототип оператора чтения из файла
	friend fstream& operator<<(fstream&, Trolleybus&);				//прототип оператора записи в файл

	static int display();											//прототип функции вывода шапки таблицы
	bool chooseParameters();										//прототип функции ввода параметров
	Trolleybus copyParameter(int);									//прототип функции копирования поля
