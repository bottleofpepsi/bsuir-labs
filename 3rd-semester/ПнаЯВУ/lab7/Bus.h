...
public:
	...
	bool operator== (const Bus&);									//прототип оператора равно
	bool operator>(const Bus&);										//прототип оператора больше
	bool operator<(const Bus&);										//прототип оператора меньше
	...
	Bus copyParameter(int);											//прототип функции копирования поля
