...
char* inputString(istream& in, int size)					//ввод строки
{
	...
	while (true)
	{
		try
		{
			...
			in.getline(buf, size, '\n');
			if (!in)
				throw InputException("ќшибка выхода за границы", "¬ведите строку до " + to_string(size) + " символов");
			return buf;
		}
		catch (InputException obj)
		{
...
char* inputCyrillicString(istream& in, int size)			//ввод строки кириллицей
{
	char* buf = new char[size + 1];
...
char* inputLatinicString(istream& in, int size)				//ввод строки латиницей
{
	char* buf = new char[size + 1];
