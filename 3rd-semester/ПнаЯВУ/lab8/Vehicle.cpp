...
float Vehicle::get_max_speed() const { return max_speed; }				        //геттеры-сеттеры
...
bool operator>(const Vehicle& obj1, const Vehicle& obj2)					    //оператор больше
{
    return obj1.max_speed > obj2.max_speed;
}

bool operator<(const Vehicle& obj1, const Vehicle& obj2)					    //оператор меньше
{
    return obj1.max_speed < obj2.max_speed;
}
...
istream& operator>>(istream& in, Vehicle& obj)						            //оператор ввода
{
    ...
    cout << "Введите модель: ";
    strcpy_s(obj.model, STR_LEN, inputString(in));
    return in;
}
