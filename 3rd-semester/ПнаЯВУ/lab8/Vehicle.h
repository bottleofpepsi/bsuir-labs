...
public:
	...
	float get_max_speed() const;									//геттеры-сеттеры
	...
	bool operator==(const Vehicle&);								//прототип оператора равно
	bool operator>(const Vehicle&);									//прототип оператора больше
	bool operator<(const Vehicle&);									//прототип оператора меньше

	friend bool operator>(const Vehicle&, const Vehicle&);			//прототип оператора больше
	friend bool operator<(const Vehicle&, const Vehicle&);			//прототип оператора меньше
	...
	Vehicle copyParameter(int);										//прототип функции копирования поля
