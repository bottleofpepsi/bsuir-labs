...
public:
	...
	Cargo(const Cargo& c) : Vehicle(dynamic_cast<const Vehicle&>(c))//конструктор копирования
	{
	...
	bool operator== (const Cargo&);									//прототип оператора равно
	bool operator>(const Cargo&);									//прототип оператора больше
	bool operator<(const Cargo&);									//прототип оператора меньше

	friend bool operator>(const Cargo&, const Cargo&);				//прототип оператора больше
	friend bool operator<(const Cargo&, const Cargo&);				//прототип оператора меньше
	...
	Cargo copyParameter(int);										//прототип функции копирования поля
